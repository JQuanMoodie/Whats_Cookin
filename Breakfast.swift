...
class BreakfastViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    private let titleLabel = UILabel()
    private let descriptionLabel = UILabel()
    private var collectionView: UICollectionView!
    private var breakfastRecipes: [Recipee] = []
    ...
    private func fetchBreakfastRecipes() {
        fetchRecipesRecursively()
    }

    private func fetchRecipesRecursively() {
        guard breakfastRecipes.count < maxRecipes else {
        ...
        recipeService.fetchRandomBreakfastRecipes { [weak self] result in
            switch result {
            case .success(let recipes):
                self?.breakfastRecipes.append(contentsOf: recipes)
                DispatchQueue.main.async {
                    self?.collectionView.reloadData()
                }
            ...
            case .failure(let error):
                print("Failed to fetch breakfast recipes: \(error)")
                DispatchQueue.global().asyncAfter(deadline: .now() + 1.0) {
                    self?.fetchRecipesRecursively()
                }
            }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let recipe = breakfastRecipes[indexPath.item]
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let selectedRecipe = breakfastRecipes[indexPath.item]
